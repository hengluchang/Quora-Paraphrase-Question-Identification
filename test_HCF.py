...
def main(argv):
	...
	MAX_NB_WORDS = 200000
	Q1_TESTING_DATA_FILE = 'q1_test_rebalanced.npy'
	Q2_TESTING_DATA_FILE = 'q2_test_rebalanced.npy'
	TESTING_HCF_FILE = 'HCF_test.npy'
	...
	if exists(Q1_TESTING_DATA_FILE) and exists(Q2_TESTING_DATA_FILE) and exists(WORD_EMBEDDING_MATRIX_FILE) and exists(NB_WORDS_DATA_FILE):
	    ...
	    HCF_data = np.load(open(TESTING_HCF_FILE,'rb'))
	else:
	...
	Q1_test = X[:,0]
	Q2_test = X[:,1]
	HCF_test = np.transpose(np.transpose(X[:,2])[1:11])
	...
	result = model.predict([Q1_test, Q2_test, HCF_test], 128, 1)
